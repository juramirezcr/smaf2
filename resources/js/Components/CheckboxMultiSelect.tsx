                ...
                <div className="absolute z-10 mt-1 w-64 rounded-md border border-gray-200 bg-white p-2 shadow-lg">
                    <div className="flex items-center justify-between gap-2">
                        ...
                        {selected.length > 0 && (
                            <button
                                type="button"
                                onClick={() => onChange([])}
                                className="shrink-0 text-xs font-medium text-indigo-600 hover:text-indigo-900"
                            >
                                Limpiar
                            </button>
                        )}
                    ...
                    <div className="mt-2 max-h-64 overflow-y-auto">
                        {filteredOptions.length === 0 ? (
                            <p className="px-1 py-1 text-sm text-gray-500">Sin resultados.</p>
                        ) : filteredOptions.map((option) => (
                            <label key={option} className="flex items-center gap-2 rounded px-1 py-1 text-sm hover:bg-gray-50">
                                <input
                                    type="checkbox"
