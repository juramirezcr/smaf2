...
export default function ImportsIndex({ batches }: ImportsProps) {
    ...
    return (
        <AuthenticatedLayout header={<h2 className="text-xl font-semibold leading-tight text-gray-800">Importaciones</h2>}>
            <Head title="Importaciones" />
            <div className="py-8">
                <div className="mx-auto max-w-7xl space-y-6 sm:px-6 lg:px-8">
                    <form onSubmit={submit} className="rounded-lg bg-white p-6 shadow-sm">
                        <h3 className="font-semibold text-gray-900">Cargar archivo CDR</h3>
                        <p className="mt-1 text-sm text-gray-500">CSV sin encabezado: ID, cuenta, origen, destino, fecha, segundos, usuario, cliente y ambiente.</p>
                        <div className="mt-4 flex flex-wrap gap-4">
                            <input type="file" accept=".csv,.log,text/csv,text/plain" onChange={(event) => setData('file', event.target.files?.[0] ?? null)} />
                        ...
                        <table className="min-w-full divide-y divide-gray-200 text-sm">
                            ...
                            <tbody className="divide-y divide-gray-200">
                                {batches.data.map((batch) => (
                                    <tr key={batch.id}>
                                        <td className="px-6 py-4"><p className="font-medium">{batch.filename}</p>{batch.failureReason && <p className="text-red-600">{batch.failureReason}</p>}</td>
